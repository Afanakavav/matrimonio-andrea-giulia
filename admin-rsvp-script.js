...
class AdminRSVPPanel {
  ...
  updateStats() {
    ...
    this.rsvpItems.forEach((item) => {
      if (item.attendance === "yes") {
        ...
        if (
          item.intolerances &&
          item.intolerances !== "Nessuna" &&
          item.intolerances.trim() !== ""
        ) {
          withIntolerances++;
        }
      } else {
  ...
  renderTable(items) {
    ...
    if (items.length === 0) {
      this.rsvpTableBody.innerHTML =
        '<tr><td colspan="9" style="text-align: center; padding: 2rem;">Nessuna prenotazione trovata</td></tr>';
      return;
    }
  ...
  viewDetails(item) {
    ...
    document.getElementById("detailsAttendance").textContent =
      item.attendance === "yes" ? "Parteciperà" : "Non parteciperà";
    document.getElementById("detailsGuests").textContent =
  ...
  exportExcel() {
    ...
    html +=
      "<tr><th>Nome</th><th>Email</th><th>Partecipazione</th><th>N. Ospiti</th><th>Intolleranze</th><th>Messaggio</th><th>Data Conferma</th></tr>";
