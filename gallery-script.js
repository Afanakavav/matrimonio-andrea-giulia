class Gallery {
  ...
  init() {
    if (!isViewingEnabled()) {
      this.showMessage(
        "La galleria e disponibile solo durante il matrimonio e per un mese successivo."
      );
      return;
    }
  ...
  openModal(item, index) {
    this.currentIndex =
      index !== undefined ? index : this.mediaItems.findIndex((i) => i.id === item.id);
  ...
  showMessage(message) {
    this.galleryContainer.innerHTML =
      '<div class="message-container"><i class="fas fa-info-circle"></i><p>' +
      message +
      "</p></div>";
    this.loadingIndicator.style.display = "none";
  }
