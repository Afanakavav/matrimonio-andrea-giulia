...
class AdminPanel {
  ...
  async downloadAll() {
    ...
    if (
      !confirm(
        `Vuoi scaricare TUTTI i ${count} file? Questa operazione potrebbe richiedere molto tempo.`
      )
    )
      return;
    ...
    try {
      ...
      if (errorCount === 0) {
        alert(
          `✓ Download di tutti i file completato! ${successCount} file scaricati con successo.`
        );
      } else {
        alert(
